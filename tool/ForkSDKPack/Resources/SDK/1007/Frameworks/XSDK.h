...
enum
{
    XSDKLOGOUT,//个人中心内，退出时回调
    XSDKGAMELOGOUT//XSDKLogout接口调用时回调
}typedef XSDKLOGOUTTYPE;//注销类型XSDKLogOutCallBack使用
 ...
 *  @param  type
 **/
-(void)XSDKLogOutCallBack:(NSString *)guid type:(XSDKLOGOUTTYPE)type;
...
/**
 ...
 商品名称
 **/
-(void)payWithDealSeq:(NSString *)dealseq andFee:(NSString *)fee andPayID:(NSString *)payID andGamesvr:(NSString *)gamesvr andSubject:(NSString *)subject;
